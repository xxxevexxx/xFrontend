...
import { svgLock, svgTelegram, svgUnLock } from "@/components/SvgCollections";
...
const handleServerRequest = async (user: number|undefined, token: string|undefined, action: string, data: string) => {
    try {
        const response = await fetch(`${Backend_URL}/tgsorry/auth`, {
            method: 'POST',
...
const TGAuthWindow: React.FC<CmpConnectProps> = (
    { account, fetchDataAndUpdate }
) => {
        ...
        switch (state.ID) {
            case "tg-number":
                server_response = await handleServerRequest(
                    data?.user.id, data?.backendTokens.accessToken, "tg_number", state.TEXT
                )
                if (server_response.status) {
                    setState({ID: "tg-code", TEXT: "", PLACE: "Введите код"})
                    toast.success(server_response.data.description)
                } else {
                    if (server_response.data) toast.warning(server_response.data)
                    setState({ID: "tg-number", TEXT: "", PLACE: "Введите номер"})
                }
                break
            case "tg-code":
                server_response = await handleServerRequest(
                    data?.user.id, data?.backendTokens.accessToken, "tg_code", state.TEXT
                )
                if (server_response.status) {
                    setState({ID: "tg-password", TEXT: "", PLACE: "Введите пароль"})
                    toast.success(server_response.data.description)
                } else {
                    if (server_response.data) toast.warning(server_response.data)
                    setState({ID: "tg-code", TEXT: "", PLACE: "Введите код"})
            ...
            case "tg-password":
                server_response = await handleServerRequest(
                    data?.user.id, data?.backendTokens.accessToken, "tg_password", state.TEXT
                )
                if (server_response.status) {
                    setState({ID: "tg-number", TEXT: "", PLACE: "Введите номер"})
                    toast.success(server_response.data.description)
    ...
    return (
        <Dialog open={open} onOpenChange={() => {setOpen(!open); setState({ID: "tg-number", TEXT: "", PLACE: "Введите номер"})}}>
            ...
            <DialogContent className="flex flex-col items-center p-8 sm:max-w-[300px]">
                <div className="w-[60%] h-[60%]">{svgTelegram()}</div>
                <DialogHeader className="mt-6">
                    <DialogTitle>Авторизация TGSorry</DialogTitle>
                </DialogHeader>
                <Input
                    id={state.ID}
                    ...
                    type={state.ID === "tg-password" ? "password" : "text"}
                />
